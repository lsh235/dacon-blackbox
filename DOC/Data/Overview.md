※ 다운로드한 baseline.zip에는 베이스라인 코드와 코드 이해를 돕기 위한 데이터 파일이 포함되어 있습니다. 
※ 해당 파일을 그대로 제출할 경우 제출 횟수가 차감되므로, 잘못 제출하지 않도록 유의해 주시기 바랍니다.
...
Stage 2 베이스라인에서는 충돌시점만 학습하며, 진입시점·회피 공간 여부·진입 방향은 별도의 정답 라벨로 학습하지 않습니다. 다만 제출 파일 생성과 전체 추론 코드의 정상 작동 여부를 확인할 수 있도록 네 항목을 모두 출력합니다. 학습되지 않은 세 항목의 결과는 제출 형식 확인을 위한 예시로, 실제 예측 성능을 의미하지 않습니다.
...
sample_index: 각 영상의 0.1초 단위 순번이며 0부터 시작
accel_label: ACCELERATING, DECELERATING, CONSTANT, STOPPED
steer_label: LEFT, STRAIGHT, RIGHT

Stage 3 베이스라인은 코드 실행 확인을 위한 소규모·희소 라벨 데이터입니다. 실제 대회 성능 확보를 위해서는 참가자가 별도의 학습데이터와 라벨링 전략을 구성해야 합니다.
...
※ 실제 평가 시에도 데이터 형태는 다음과 같이 유지됩니다.
Stage 1: 영상 파일
Stage 2: 영상별 폴더로 구분된 연속 프레임 이미지
Stage 3: 10Hz 영상 파일
※ 참가자는 비공개 평가데이터의 전체 수량, 정답 및 원천정보를 확인할 수 없습니다. 제출한 추론 코드는 평가 서버에서 비공개 데이터를 입력받아 Stage별 예측 결과를 생성합니다.
